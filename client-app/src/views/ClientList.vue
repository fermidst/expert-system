<template>
  <MainLayout>
    ...
    <v-data-table
      ...
      disable-pagination
      hide-default-footer
      :loading="clientListInfo.isLoading"
    >
      <template #top>
        <v-toolbar flat>
        ...
        <v-expansion-panels accordion>
          <v-expansion-panel>
            <v-expansion-panel-header
              >Информация о купленных ювелирных изделиях
              клиента</v-expansion-panel-header
            >
            <v-expansion-panel-content>
              <v-container>
                <v-row>
                  <v-col>
                    Название:
                    {{ jewelries.find(j => j.id === item.jewelryId).name }}
                  </v-col>
                </v-row>
                <v-row>
                  <v-col>
                    Цена:
                    {{ jewelries.find(j => j.id === item.jewelryId).price }}
                    рублей
                  </v-col>
                </v-row>
                <v-row>
...
export default {
  ...
  computed: {
    ...
    editedItemJewelryId: {
      get() {
        return this.$store.state.client.editedItem.jewelryId;
      },
      set(value) {
  ...
  methods: {
    ...
    postItem() {
      this.editedItemJewelryId = this.selectedJewelry.id;
    ...
    clearFields() {
      ...
      this.selectedJewelry = Object.assign({}, this.defaultJewelry);
    }
  }
};
</script>
